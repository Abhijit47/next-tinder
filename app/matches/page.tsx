...
import { getPotentialMatches, likeUser } from '@/lib/matches-actions';
import { useRouter } from 'next/navigation';
import { useEffect, useState } from 'react';
import MatchButtons from './_components/match-buttons';
import MatchCard from './_components/match-card';
import MatchNotification from './_components/match-notification';

export default function MatchesPage() {
  const [potentialMatches, setPotentialMatches] = useState<InsertUser[]>([]);
  const [loading, setLoading] = useState(true);
  const [currentIndex, setCurrentIndex] = useState(0);

  const [showMatchNotification, setShowMatchNotification] = useState(false);
  const [matchedUser, setMatchedUser] = useState<InsertUser | null>(null);
