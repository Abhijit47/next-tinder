...
interface EMessageResponseBase extends MessageResponse {
  caller_id?: string;
  caller_name?: string;
  call_id?: string;
}
...
export default function StreamChatInterface({
        ...
        chatChannel.on('message.new', (event: Event) => {
          ...
          if (event.message) {
            if (event.message.text?.includes(`📹 Video call invitation`)) {
              const customData = event.message as EMessageResponseBase;
    ...
    try {
      ...
      if (channel) {
        const messageData = {
          ...
          caller_name: otherUser.fullName || 'Someone',
        };
  ...
  //           <p className='text-lg'>
  //             {isCallInitiator ? 'Starting call...' : 'Loading chat...'}
  //           </p>
  //         </div>
  //       </div>
  //     </div>
          ...
          <div className='bg-white dark:bg-gray-800 rounded-2xl p-8 max-w-sm mx-4 shadow-2xl'>
            <div className='text-center'>
              <div className='w-20 h-20 rounded-full overflow-hidden mx-auto mb-4 border-4 border-pink-500'>
                <Image
                  src={
                    otherUser.avatarUrl ?? 'https://placehold.co/600x400/png'
                  }
